...
def summarize(text: str) -> str:
    """Summarize a long passage of text using the Mistral model."""
    prompt = f"Summarize the following text: {text}"
    try:
        response = llm.invoke(prompt)
        return response.content.strip()
    except Exception as e:
        return f"Error: Could not summarize the text. {str(e)}"
...
while True:
    ...
    if user_input.lower().startswith("weather"):
        try:
            ...
            if not city:
                print("Agent: Please specify a city.")
                continue
    ...
    if user_input.lower().startswith("summarize"):
        try:
            ...
            print("Agent: Could not summarize the text:", e)
            continue
